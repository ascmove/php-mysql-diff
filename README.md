...
To install PHP MySQL Diff, install Composer using the following command:
...
```
$ mysqldump -h hostname -u username -p password -d dbname > outputfile.sql
```
...
* I failed to execute the command “php-mysql-diff diff <from> <to> [-i <ignore-tables-file>]”
    If you are root user,may this will be helpful:
  ```
  php /root/.composer/vendor/camcima/php-mysql-diff/php-mysql-diff diff fromscript.sql toscript.sql
  ```
